...
public class ShushHand : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hitInfo, playerReach))
        {
            ...
            if (hitInfo.collider.GetComponent<NoiseStation>())
            {
                NoiseStation station = hitInfo.collider.GetComponent<NoiseStation>();
                ...
                if (station.isEnabled)
                {
                    ...
                    {
                        station.DecreaseNoise(Time.deltaTime * decreasePerSecond);
                        shushText_gameObject.SetActive(false);
                    }
                    else
                    {
                        station.isBeingShushed = false;
                    }
                }
            }
            else
